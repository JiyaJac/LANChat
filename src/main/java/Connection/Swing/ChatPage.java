...
public class ChatPage extends UITheme {
    ...
    private void fetchBroadcastHistory() {
        if (ps == null) return;
        ps.println("FETCH_BROADCAST:" + username);
    }
    ...
    private void fetchPrivateHistory(String otherUser) {
        if (ps == null) return;
        // Send request to server
        ps.println("FETCH_PRIVATE:" + username + ":" + otherUser);
    }
        ...
        new Thread(() -> {
            try {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    else if (line.startsWith("BROADCAST_HISTORY:")) {
                        String[] parts = line.split(":", 3);
                        if (parts.length >= 3) {
                            String sender = parts[1];
                            String msg = parts[2];
                    ...
                    else if (line.startsWith("PRIVATE_HISTORY:")) {
                        String[] parts = line.split(":", 4);
                        if (parts.length >= 4) {
                            String sender = parts[1];
                            String recipient = parts[2];
